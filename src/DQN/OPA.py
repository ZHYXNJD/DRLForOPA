...
class OPA:
    ...
    def step(self, action, invalid_action):
        """
        ...
        if self.request_num <= self.total_request:
            if action == sd.total_slot:
                ...
                self.rewards = self.alpha1 * revenue[self.request_num - 1] - self.alpha3 * 0.01 / 1.2 * \
                               lzd[destination[self.request_num - 1] - 1][action]

                # 更新states
                self.states["supply"][action] = self.states["supply"][action] + self.states["demand"]
            if self.request_num < self.total_request:
                next_states = {"demand": rmk[self.request_num], "type": req_type[self.request_num],
